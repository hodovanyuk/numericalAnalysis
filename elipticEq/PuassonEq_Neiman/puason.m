%уравнение Пуасона з условием Неимана с производнои
clc
clear all
nx = 100;
ny = nx;
...
b(2,:) = b(2,:) + ax(2,:);  % производная по х = 0
d(2,:) = d(2,:) + ax(2,:)*hx; %производная по х = 1, если добавляется эта строка
ax(2,:) = 0;                % производная по х = 0
...
U(1,:) = U(2,:); % обрезаем значения стенки (1,:), что бы не было стенки
